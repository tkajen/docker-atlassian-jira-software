...
RUN addgroup jirauser \
    && useradd -r -u 999 -g jirauser jirauser
...
RUN set -x \
    ...
    && chown -R jirauser:jirauser  "${JIRA_INSTALL}/conf" \
    && chown -R jirauser:jirauser  "${JIRA_INSTALL}/logs" \
    && chown -R jirauser:jirauser  "${JIRA_INSTALL}/temp" \
    && chown -R jirauser:jirauser  "${JIRA_INSTALL}/work" \
